...
object Database {
	...
	def SaveUser(user: User) {
		try {
			val query = s"UPDATE users SET username = '${user.GetUsername()}', password = '${user.GetPassword()}', balance = ${user.GetBalance()} WHERE id = ${user.GetID()};"
	...
	def GetUser(name: String): Option[User] = {
		try {
			searchUser(name) match {
				case Some(res) => {
					if (res.next()) {
						val usr = new User(res.getInt(1), res.getString(2), res.getString(3))
						Some(usr)
					} else {
						Some(new User())
					}
	...
	def InsertNewUser(user: User) {
		try {
			val query = s"INSERT INTO users(username, password, balance) values ('${user.GetUsername()}', '${user.GetPassword()}', ${user.GetBalance()});"
			this.ExecuteUpdate(query)
		} catch {
